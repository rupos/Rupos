 ...
 * @author hverbeek
 */
public class XpdlElement implements Cloneable{
	...
	public Object clone() {
		XpdlElement ele = new XpdlElement(tag);
		ele.lineNumber = lineNumber;
			...
			try {
				...
				if (eventType == XmlPullParser.END_DOCUMENT) {
					...
					if (xpp.getName().equals(stack.get(stack.size() - 1))) {
						 ...
						 */
						xpdl.log(tag, xpp.getLineNumber(), "Found " + xpp.getName() + ", expected "
	...
	protected void checkBoolean(Xpdl xpdl, String label, String value, boolean isRequired) {
		checkRestriction(xpdl, label, value, Arrays.asList("true", "false", "1", "0"), isRequired);
	}
